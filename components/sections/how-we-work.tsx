...
const guarantees: Guarantee[] = [
  {
    ...
    body: 'Both of us run the work and the calls. No account-manager telephone game.',
  },
  {
    icon: Package,
          ...
          {guarantees.map((item, index) => {
            ...
            return (
              <Reveal key={item.title} delay={index * 0.1}>
                <Card variant="inverted" className="flex h-full flex-col p-6">
                  <IconCircle icon={Icon} size="sm" className="mb-4 shrink-0" />
                  <Headline as="h4" className="mb-2 text-base">
                    {item.title}
                  </Headline>
                  <BodyText className="text-sm">{item.body}</BodyText>
                </Card>
              </Reveal>
            )
          })}
