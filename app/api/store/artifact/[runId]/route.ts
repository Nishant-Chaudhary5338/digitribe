...
import { buildFileBundle, type BundleFile } from '../../../../../lib/store/bundle'

function extractFiles(artifact: unknown): BundleFile[] {
  if (
    artifact &&
    ...
    return (artifact as { files: unknown[] }).files.filter(
      (f): f is BundleFile =>
        !!f &&
        typeof f === 'object' &&
        typeof (f as BundleFile).path === 'string' &&
        typeof (f as BundleFile).contents === 'string'
    )
  }
  return []
}
...
export async function GET(
  ...
  try {
    ...
    if (req.nextUrl.searchParams.get('fmt') === 'zip') {
      const files = extractFiles(artifact)
      if (files.length === 0)
        throw storeError('INPUT_INVALID', 'This result has no downloadable files.')
      const zip = await buildFileBundle(files, 'Generated by Digitribe — digitribe.world/store')
      return new NextResponse(new Uint8Array(zip), {
        headers: {
          'Content-Type': 'application/zip',
          'Content-Disposition': `attachment; filename="${run.slug}-bundle.zip"`,
        },
      })
    }
